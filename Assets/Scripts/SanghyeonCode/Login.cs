...
public class Login : MonoBehaviour
{
    ...
    public void StartPrepareStart()
    {
        // Post prepare request키 받아오기
        StartCoroutine(GetRequestKey());
    }

    private IEnumerator GetRequestKey()
    {
        ...
        else
        {
            ...
            Status convertJson = JsonUtility.FromJson<Status>(result);
            Debug.Log(convertJson.request_key);
    ...
    public void GenerateQR()   // QR코드 생성 함수
    {
        int width = 256;
        int height = 256;
        Debug.Log(getData);
